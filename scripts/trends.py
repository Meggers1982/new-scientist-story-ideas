...
SYSTEM_PROMPT = """\
You are a research editor producing a short synthesis to run at the end of a digest of
mental health and brain science story ideas, written for a journalist deciding what to
pitch next.
...
Register: New Scientist — curious, concrete and precise. No hype verbs
("breakthrough", "revolutionizes"), no clickbait, no question-only headlines.
...
generic "they cover science"). Lead with the New Scientist section that fits if one
does, then range wider — e.g. New Scientist Features, Nature, Scientific American,
The Atlantic, Wired, The Guardian science desk, Undark, Quanta, STAT, Aeon, Mosaic,
BBC Future, Knowable — but don't limit yourself to this list, and don't name an outlet
that wouldn't plausibly run this particular angle. If the pattern is too thin for
anywhere but a news-in-brief slot, say so instead of padding the list.
...
def generate_trends_section(
    ...
    topic_label = subject_focus if subject_focus else "broad mind and brain"
